<template>
  <div class="space-y-3">
    ...
    <div v-if="loading" class="text-center py-8">
      <LoadingSpinner size="sm" text="加载查询历史..." />
    </div>
          ...
          <div class="flex items-start justify-between gap-3">
            ...
            <div class="flex-1 min-w-0">
              <div class="flex items-center gap-2 mb-2">
                ...
                <span v-if="item.duration" class="text-xs text-base-content/60">
              ...
              <div class="mb-2">
                ...
                <p v-if="item.generatedSql" class="text-xs text-base-content/60 mt-1 font-mono line-clamp-1">
                  {{ item.generatedSql }}
                </p>
              </div>

              <!-- Result Summary -->
              <div v-if="item.result" class="text-xs text-base-content/60">
                返回 {{ item.result.rowCount || 0 }} 行数据
              </div>

              <!-- Error Message -->
              <div v-if="item.errorMessage" class="text-xs text-error mt-2">
                {{ item.errorMessage }}
              </div>
            </div>
