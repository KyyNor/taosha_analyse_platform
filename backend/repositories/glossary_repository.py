...
class GlossaryTermRepository(BaseRepository[GlossaryTerm]):
    ...
    def __init__(self, db: Session):
        super().__init__(GlossaryTerm, db)
    ...
    def get_by_type(self, term_type: str) -> List[GlossaryTerm]:
        """根据类型获取术语列表"""
        try:
            return self.db.query(GlossaryTerm).filter(GlossaryTerm.type == term_type).all()
        except Exception as e:
    ...
    def get_by_creator(self, creator: str) -> List[GlossaryTerm]:
        """根据创建者获取术语列表"""
        try:
            return self.db.query(GlossaryTerm).filter(GlossaryTerm.creator == creator).all()
        except Exception as e:
    ...
    def search_terms(self, query: str) -> List[GlossaryTerm]:
        """搜索术语（按名称或内容）"""
        try:
            return self.db.query(GlossaryTerm)\
                     .filter(
                         or_(
