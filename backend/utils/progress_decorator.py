...
def track_node_progress(node_name: str):
    ...
    def decorator(func: Callable) -> Callable:
        ...
        def wrapper(state: Dict[str, Any]) -> Dict[str, Any]:
            ...
            # 记录节点开始
            start_log = progress_tracker.log_node_start(node_name, state, progress_callback)
            ...
            node_log_entry = {
                ...
                'input_data': start_log['input_data'],
                'start_time': start_log['timestamp']
            }
            logs.append(node_log_entry)
            ...
            try:
                ...
                # 记录节点完成
                end_log = progress_tracker.log_node_end(node_name, result_state, progress_callback)
                ...
                completed_log = {
                    **node_log_entry,
                    ...
                    'output_data': end_log['output_data'],
                    'end_time': end_log['timestamp'],
                    'success': True
                }

                # 替换最后一个日志条目
                if logs and logs[-1]['step'] == node_name and logs[-1]['status'] == 'running':
                    logs[-1] = completed_log
                else:
                    logs.append(completed_log)

                # 更新状态中的日志
                result_state['logs'] = logs
                ...
                error_log = {
                    **node_log_entry,
                    'status': 'failed',
                    'message': f'{node_name}流程失败: {str(e)}',
                    'error': str(e),
                ...
                progress_tracker.logger.error(f"节点失败: {node_name}, 错误: {str(e)}")

                # 更新进度回调（如果有）
                if progress_callback:
                    progress_callback(node_name, f'{node_name}流程失败', 0)
