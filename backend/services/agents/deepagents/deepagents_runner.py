...
- 直接在轮询线程中执行任务（无子进程）
- 通过 ModelCallLimitMiddleware 防止无限循环
- 状态追踪和结果保存
"""
...
def _run_analysis_task(session_id: str, question: str):
    """执行分析任务（直接调用，无需子进程）
    ...
    try:
        ...
        if analyse_result.get("success"):
            ...
            _update_session_status(session_id, "failed", llm_output=analyse_result.get('error'))
        ...
        _update_session_status(session_id, "failed", llm_output=f"任务执行异常: {str(e)}")
...
class DeepAgentsRunner:
    ...
    直接在轮询线程中执行任务，通过数据库状态判断是否有任务在运行。
    """
    ...
    def __init__(self):
        ...
        logger.info("DeepAgentsRunner 初始化完成")
    ...
    def _execute_task(self, session_id: str, question: str):
        ...
        """
        try:
            _run_analysis_task(session_id, question)
        except Exception as e:
