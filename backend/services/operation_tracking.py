...
class OperationTracker:
    ...
    def start_session(self, task_id, operation_type: str, operator: str = None) -> str:
        """开始一个新的操作会话"""
        session_id = task_id
...
@contextmanager
def track_operation(task_id: str,operation_type: str, operator: str = None):
    """操作追踪上下文管理器"""
    task_id = tracker.start_session(task_id, operation_type, operator)
    try:
        yield task_id
        tracker.end_session(task_id)
    except Exception as e:
        tracker.end_session(task_id, str(e))
        raise
